...
export const useAuthStore = create<AuthState>((set, get) => ({
  user: isClient ? JSON.parse(localStorage.getItem(LOCAL_STORAGE_USER_KEY) || 'null') : null,
  token: isClient ? localStorage.getItem(LOCAL_STORAGE_TOKEN_KEY) : null,
  serverUrl: (isClient ? localStorage.getItem(LOCAL_STORAGE_URL_KEY) : null) || 'http://localhost:3000',
  authStrategy: ((isClient ? localStorage.getItem(LOCAL_STORAGE_STRATEGY_KEY) : null) as 'JWT' | 'SESSION') || 'JWT',
  serverHealth: null,
  isConnecting: false,
